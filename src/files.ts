...
export function parseFileIgnores(path: string): string[] {
  ...
  try {
    ...
    if (path.includes(DOTKHULNASOFT_FILENAME)) {
 ...
 *
 * Exclude rules from .khulnasoft files and ignore rules from .[*]ignore files are collected separately.
 * Any .[*]ignore files in paths excluded by .khulnasoft exclude rules are ignored.
 */
export async function collectFilePolicies(
  ...
  const tasks = dirs.map(async folder => {
    ...
    // Find .khulnasoft and .[*]ignore files inside this directory.
    ...
    // Parse rules from all .khulnasoft files inside this directory.
    const khulnasoftFiles = allIgnoredFiles.filter(f => f.endsWith(DOTKHULNASOFT_FILENAME));
    const khulnasoftExcludeRules = union(...khulnasoftFiles.map(parseFileIgnores));

    // Parse rules from relevant .[*]ignore files inside this directory.
    // Exclude ignore files under paths excluded by .khulnasoft files.
    const ignoreFiles = allIgnoredFiles.filter(
      f => !f.endsWith(DOTKHULNASOFT_FILENAME) && multimatch([nodePath.dirname(f)], khulnasoftExcludeRules).length === 0,
    );
    const ignoreFileRules = union(...ignoreFiles.map(parseFileIgnores));

    return {
      excludes: khulnasoftExcludeRules,
