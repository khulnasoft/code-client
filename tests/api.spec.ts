...
describe('Requests to public API', () => {
  it('gets filters successfully', async () => {
    ...
    expect(new Set(response.value.configFiles)).toEqual(new Set(['.dcignore', '.gitignore', '.khulnasoft', '.khulnasoft-rules']));
  ...
  it('test network issues', async () => {
    const response = await getFilters('https://faketest.khulnasoft.com', 'test-source', 1);
